...
def mock_ai_response(question):
    ...
    # Конкретні плетіння - нова категорія з 4-ї партії
    if any(word in question_lower for word in ["граненій якір", "якірне плетіння", "граненый"]):
        return "Вітаю. Ціна плетіння Граненій якір по 155грн/г готового виробу з замком карабіном. Замок-коробочка по доплаті у 600грн. При масі 50г вартість складе 7,750 грн."
    
    # Прийом лому срібла
    if any(word in question_lower for word in ["срібло", "лом", "здати", "по скільки", "приймає"]):
        return "Приймаємо лом срібла 925 проби по 85-87грн/г. Зараховуємо в рахунок нового замовлення. Відправка лому - перед початком виготовлення."
    
    # Додаткові покриття
    if any(word in question_lower for word in ["родій", "родіювання", "покривання", "позолота"]):
        if "позолота" in question_lower:
            return "Вітаю, з позолотою зараз не працюємо."
        return "Родіювання по 95грн/г від маси виробу. Це додаткова послуга до основної вартості плетіння."
    
    # Комбіновані замовлення
    if any(word in question_lower for word in ["замовлення", "цепочку", "здати срібло"]) and any(word in question_lower for word in ["80", "лом", "рахунок"]):
        return "Орієнтовна вартість ланцюжка 80г складатиме 21,400 грн (без урахування вашого срібла). Прийом лома по 87грн/г в 925°. Множимо масу лому на 87 та віднімаємо від кінцевого розрахунку."
    
    # Логістика лому
    if any(word in question_lower for word in ["адреса", "куди відправляти", "лом", "пошта"]):
        return "В нас відправка лому перед початком виготовлення замовлення. Адресу надам після підтвердження замовлення та узгодження термінів."
    
    # Тайминг процесів  
    if any(word in question_lower for word in ["зараз треба", "коли відправляти", "почекати"]):
        return "А вам саме зараз треба? Бо ми перед початком роботи кажемо за відправлення. Щоб зайве в нас по часу не лежало в сейфі. Краще синхронізувати."
    
    # Прийом до виготовлення
    if any(word in question_lower for word in ["готуйде", "поки цепочку", "прийнято"]):
        return "Прийнято до виготовлення! Плетіння Якірне, ланцюжок 60см, маса ~80 грам (165грн/г). Уточню терміни та процедуру відправки лому."
